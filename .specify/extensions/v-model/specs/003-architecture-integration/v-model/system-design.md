...
This system design decomposes the requirements for v0.3.0 of the V-Model Extension Pack into 13 system components organized across the extension's artifact types: Markdown agent prompts (commands), Markdown output structure definitions (templates), deterministic Bash/PowerShell helpers (scripts), Python-based CI test infrastructure (validators/evals), the extension manifest, and domain overlay infrastructure. The decomposition follows the natural boundary of each deliverable artifact — commands that orchestrate AI generation, templates that constrain output structure, scripts that perform deterministic validation and matrix building, the overlay assembly protocol that enables domain-specific extensibility, and the manifest that registers capabilities. Two commands (architecture-design, integration-test) form the core of this V-level, supported by a validation script, template pair, trace command extension, matrix builder extensions, setup script extensions, a manifest update, CI evaluation suite extensions, and a domain overlay assembly module.

**Evolution History (Overlay Evolution Wave):** This document was evolved from the original 12-component design to incorporate overlay-aware domain extensibility (REQ-050 through REQ-055, REQ-NF-006). Lifecycle suspect analysis was performed on all SYS components tracing to deprecated parent requirements:

- **SYS-001, SYS-004**: Suspect from REQ-021 (deprecated → REQ-050). Resolved: re-parented to REQ-050; REQ-021 removed from parents.
- **SYS-002, SYS-005**: Suspect from REQ-022 (deprecated → REQ-051). Resolved: re-parented to REQ-051; REQ-022 removed from parents.
- **SYS-001, SYS-002**: Suspect from REQ-CN-001 (deprecated → REQ-054). Resolved: re-parented to REQ-054; REQ-CN-001 removed from parents.
- **SYS-006**: Suspect from REQ-047 (deprecated). Resolved: capability subsumed by REQ-035 (progressive matrix building); REQ-047 removed from parents. SYS-006 confirmed active.
- **SYS-003**: Suspect from REQ-048 (deprecated). Resolved: partial execution capability withdrawn; REQ-048 removed from parents. SYS-003 confirmed active with remaining parents.

New component SYS-013 (Domain Overlay Infrastructure) was added to implement the assembly protocol (REQ-055) and overlay-only extensibility (REQ-NF-006).
...
| SYS-013 | Domain Overlay Infrastructure | Domain overlay infrastructure module that defines and implements the assembly protocol for loading domain-specific content into V-Model generative commands. When `v-model-config.yml` specifies a `domain` value (e.g., `iso_26262`, `do_178c`, `iec_62304`), this module provides the directory structure conventions and discovery mechanism for loading domain-specific command overlays from `commands/overlays/{domain}/{command}.md` and template overlays from `templates/overlays/{domain}/{template}.md`. Overlay content is applied alongside base commands and templates via the assembly protocol, ensuring domain-specific sections (e.g., ASIL decomposition, SIL/HIL compatibility) are included only when relevant. Designed for zero-modification extensibility: adding support for a new regulated domain requires only creating new overlay files in the appropriate directories — no changes to base commands, base templates, or the assembly protocol itself. | REQ-055, REQ-NF-006 | Module |
...
| SYS-001 | SYS-013 | Uses | Architecture design command cannot discover or load domain overlay files; when a domain is configured, output will lack domain-specific safety-critical sections. Non-domain projects are unaffected. |
...
| SYS-002 | SYS-013 | Uses | Integration test command cannot discover or load domain overlay files; when a domain is configured, output will lack domain-specific safety-critical test sections. Non-domain projects are unaffected. |
...
| SYS-013 | SYS-004 | Reads | Domain overlay infrastructure reads base template to determine overlay merge points; if template structure changes, overlay conventions must be updated. |
| SYS-013 | SYS-005 | Reads | Domain overlay infrastructure reads base template to determine overlay merge points; if template structure changes, overlay conventions must be updated. |
...
graph TD
    ...
    SYS-013 -.->|Reads| SYS-004
    SYS-013 -.->|Reads| SYS-005
```
...
| SYS-001 | SYS-013 | Domain Overlay Loading (Architecture) | File read | Command overlay: `commands/overlays/{domain}/architecture-design.md` — Markdown with domain-specific architecture sections (e.g., ASIL decomposition, defensive programming). Template overlay: `templates/overlays/{domain}/architecture-design-template.md` — domain-specific template sections. | Overlay file not found for configured domain: command proceeds with base-only output and logs a warning. No domain configured: overlay loading skipped entirely. |
...
| SYS-002 | SYS-013 | Domain Overlay Loading (Integration Test) | File read | Command overlay: `commands/overlays/{domain}/integration-test.md` — Markdown with domain-specific test sections (e.g., SIL/HIL compatibility, resource contention). Template overlay: `templates/overlays/{domain}/integration-test-template.md` — domain-specific template sections. | Overlay file not found for configured domain: command proceeds with base-only output and logs a warning. No domain configured: overlay loading skipped entirely. |
...
| SYS-013 | SYS-004 | Overlay Merge Point Discovery (Architecture Template) | File read | Reads base `architecture-design-template.md` to identify conditional safety-critical section placeholders where domain overlay content is merged | Template missing or lacking merge points: overlay content cannot be positioned — assembly protocol logs warning and skips domain sections. |
| SYS-013 | SYS-005 | Overlay Merge Point Discovery (Integration Test Template) | File read | Reads base `integration-test-template.md` to identify conditional safety-critical section placeholders where domain overlay content is merged | Template missing or lacking merge points: overlay content cannot be positioned — assembly protocol logs warning and skips domain sections. |
...
| Domain Command Overlays | SYS-013 | File (Markdown in `commands/overlays/{domain}/`) | Repository access controls; read-only at runtime by generative commands via assembly protocol | N/A — local file system; protected by repository transport | Permanent — part of the extension distribution. Each domain's overlay files are versioned with the extension. New domains added by creating files only. |
| Domain Template Overlays | SYS-013 | File (Markdown in `templates/overlays/{domain}/`) | Repository access controls; read-only at runtime by generative commands via assembly protocol | N/A — local file system; protected by repository transport | Permanent — part of the extension distribution. Each domain's template overlay files are versioned with the extension. New domains added by creating files only. |
| v-model-config.yml | SYS-013 | File (YAML at repository root) | Repository access controls | N/A — local file system | User-managed; persists until user modifies or removes. Absence means no domain configuration — commands produce general-purpose output. |
...
| Metric | Count |
|--------|-------|
| Total System Components (SYS) | 13 (13 active, 0 deprecated, 0 suspect) |
| Total Parent Requirements Covered | 63 / 68 (92.6%) (5 deprecated REQs — REQ-021, REQ-022, REQ-047, REQ-048, REQ-CN-001 — excluded from SYS parents per lifecycle rules) |
| Components per Type | Subsystem: 0 \| Module: 8 \| Service: 0 \| Library: 0 \| Utility: 5 |
| **Forward Coverage (REQ→SYS)** | **95.6%** (100% of active REQs covered; 3 deprecated REQs intentionally excluded from SYS parents) |
...
| Domain Overlay | A domain-specific Markdown file loaded from `commands/overlays/{domain}/` or `templates/overlays/{domain}/` that provides safety-critical or regulatory content to supplement base commands and templates. |
| Assembly Protocol | The standardized mechanism by which generative commands discover and load domain overlay files based on the `domain` value in `v-model-config.yml`. Ensures overlay-only extensibility. |
| v-model-config.yml | User-managed YAML configuration file at the repository root. When `domain` is set (e.g., `iso_26262`), triggers domain overlay loading. Absence means no domain configuration. |
