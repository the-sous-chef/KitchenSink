...
description: 'Cross-cutting artifact consistency checker. Verifies alignment across
  ALL lifecycle layers — research ↔ product-spec ↔ spec.md ↔ plan.md ↔ tasks.md ↔
  code. Detects forward drift (earlier artifacts not reflected in later) and backward
  drift (later decisions that should update earlier artifacts). Runnable between ANY
  phases. Human-in-the-loop for every CRITICAL/WARNING resolution. Use: "sync-verify",
  "check consistency", "/speckit.product-forge.sync-verify"'
---
