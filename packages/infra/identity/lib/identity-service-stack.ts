...
export class IdentityServiceStack extends Stack {
    public constructor(scope: Construct, id: string, props: IdentityServiceStackProps) {
        ...
        const imageTag = this.node.tryGetContext('identityImageTag') ?? process.env.IDENTITY_IMAGE_TAG ?? 'latest';
        const desiredCount = Number(
            this.node.tryGetContext('identityDesiredCount') ?? process.env.IDENTITY_DESIRED_COUNT ?? 2,
        );
        ...
        props.data.auth0ManagementSecret.grantRead(taskRole);
        ...
        taskDefinition.addContainer('IdentityServiceContainer', {
            ...
            secrets: {
                ...
                AUTH0_DOMAIN: ecs.Secret.fromSecretsManager(props.data.auth0ManagementSecret, 'domain'),
                AUTH0_AUDIENCE: ecs.Secret.fromSecretsManager(props.data.auth0ManagementSecret, 'audience'),
                AUTH0_CLIENT_ID: ecs.Secret.fromSecretsManager(props.data.auth0ManagementSecret, 'clientId'),
                AUTH0_CLIENT_SECRET: ecs.Secret.fromSecretsManager(props.data.auth0ManagementSecret, 'clientSecret'),
            },
            portMappings: [
                {
                    containerPort: 3000,
