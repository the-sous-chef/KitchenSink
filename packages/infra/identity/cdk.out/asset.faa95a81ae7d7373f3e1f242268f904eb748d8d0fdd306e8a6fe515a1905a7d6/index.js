...
var f = Object.create,
    ...
    d = (t, e, o, r) => {
        if ((e && typeof e == 'object') || typeof e == 'function')
            for (let s of C(e))
                !P.call(t, s) && s !== o && i(t, s, { get: () => e[s], enumerable: !(r = I(e, s)) || r.enumerable });
        return t;
    },
    l = (t, e, o) => (
        (o = t != null ? f(w(t)) : {}),
        d(e || !t || !t.__esModule ? i(o, 'default', { value: t, enumerable: !0 }) : o, t)
    ),
...
var h = require('@aws-sdk/client-s3'),
    y = l(require('https')),
    m = l(require('url')),
    a = { sendHttpRequest: D, log: T, includeStackTraces: !0, userHandlerIndex: './index' },
    p = 'AWSCDK::CustomResourceProviderFramework::CREATE_FAILED',
    L = 'AWSCDK::CustomResourceProviderFramework::MISSING_PHYSICAL_ID';
function R(t) {
    return async (e, o) => {
        let r = { ...e, ResponseURL: '...' };
        if ((a.log(JSON.stringify(r, void 0, 2)), e.RequestType === 'Delete' && e.PhysicalResourceId === p)) {
            (a.log('ignoring DELETE event caused by a failed CREATE event'), await u('SUCCESS', e));
            return;
        }
        try {
            let s = await t(r, o),
            ...
            (n.PhysicalResourceId ||
                (e.RequestType === 'Create'
                    ...
                    : a.log(`ERROR: Malformed event. "PhysicalResourceId" is required: ${JSON.stringify(e)}`)),
                await u('FAILED', n));
        }
    };
}
...
async function u(t, e) {
    ...
    let n = JSON.stringify(o),
        E = {
            ...
            path: r.path,
            method: 'PUT',
            headers: { 'content-type': '', 'content-length': Buffer.byteLength(n, 'utf8') },
        };
...
async function _(t) {
    try {
        ...
        console.log(`Could not set new object deny policy on bucket '${t}' prior to deletion.`);
    }
}
async function U(t) {
    let e;
...
async function W(t) {
    return (await c.getBucketTagging({ Bucket: t })).TagSet?.some((o) => o.Key === g && o.Value === 'true');
}
