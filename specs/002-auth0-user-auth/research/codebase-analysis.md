# Codebase Analysis: Auth0 User Authentication
...
Web authentication integration surface (Next.js + `@auth0/nextjs-auth0`) with protected-route redirects and callback handling.

### `packages/apps/sous-chef/mobile`

Mobile authentication integration surface (Expo + `react-native-auth0`) with secure storage (`expo-secure-store`) and deep-link callback handling.
...
| `src/auth/webhook/*`        | Auth0 post-registration action ingestion and DB sync                         |
...
| `src/auth/reconciliation/*` | Nightly Auth0↔DB consistency repair job                                      |
...
- **User** (`id`, `auth0_id`, `email`, `display_name`, `avatar_url`, `status`, timestamps)
- **Account** (subscription/account metadata linked to user)

Critical consistency invariants:

1. `User.id` (UUIDv4) is canonical and must be propagated through Auth0 metadata claim flows (FR-015, FR-040).
2. Deletion is cascade-based for all user-owned data paths (FR-025).
3. Suspension state is dual-tracked (Auth0 block + DB status) for defense-in-depth (FR-041..FR-044).
...
- Account lifecycle updates (profile/edit/suspension/reactivation/deletion) remain DB-backed with Auth0 as identity authority.

### Auth Boundary to Async Infrastructure

- SQS queue + DLQ handle eventual consistency for Auth0 deletion API failures.
