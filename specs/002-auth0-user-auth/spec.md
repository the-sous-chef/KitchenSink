...
> **Added 2026-05-14 (T18 doc-sync):** The implementation uses a locked dual-runtime split. This section records the authoritative boundary so downstream V-Model artifacts and implementation tasks can reference it unambiguously.

| Runtime                                 | Package                                | Responsibilities                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------------------------------------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **ECS/Fargate — NestJS 11 (Node 24)**   | `packages/services/identity/`          | REST API for user/account/profile CRUD (`GET/PATCH /v1/users/me`, `DELETE /v1/users/me`, `GET/PATCH /v1/users/me/profile`, `POST/DELETE /v1/users/me/social-link`, `GET /v1/admin/users/:id`, `POST /v1/admin/users/:id/suspend`, `POST /v1/admin/users/:id/unsuspend`, impersonation endpoints). Receives `AuthorizerContext` via trusted internal header injected by the API Gateway authorizer. Never accepts raw JWT claims from clients. |
| **AWS Lambda (Node 22) — raw handlers** | `packages/services/identity-webhooks/` | API Gateway REQUEST authorizer (JWT verification, suspension check, IAM policy emission); Auth0 post-registration webhook (user/account provisioning); SQS deletion worker (async Auth0 + DB deletion with DLQ); nightly reconciliation (EventBridge-triggered).                                                                                                                                                                              |
| **Shared contracts**                    | `packages/shared/auth-types/`          | TypeScript interfaces, Drizzle schema, queue message shapes, error types — imported by both runtimes. Single source of truth for the `users`, `accounts`, `profiles` table definitions.                                                                                                                                                                                                                                                       |
| **IaC**                                 | `packages/infra/identity/`             | CDK v2 for VPC, RDS, SQS, S3, ECS cluster/service/ALB, ECR, Secrets Manager, CloudWatch alarms. Serverless Framework (`serverless.yml`) for Lambda functions and API Gateway wiring.                                                                                                                                                                                                                                                          |
...
- **User**: Represents a registered Sous Chef user stored in our database. Key attributes: `id` (varchar(255) COLLATE "C" — the canonical identifier across all Sous Chef systems, containing the Auth0 `sub` claim value, e.g., `auth0|abc123`), `email` (synced from Auth0 at registration), `displayName`, `avatarUrl` (nullable), `status` (enum: `active` | `suspended` — default: `active`; suspended users are blocked in Auth0 and denied API access; status is managed by backend admin operations), `createdAt` (ISO 8601), `updatedAt` (ISO 8601). The User entity is the owner of all user-generated content (recipes, meal plans, etc.) and is the target of the cascade delete on account deletion. This entity fulfills Sous Chef FR-045 ("System MUST require user authentication for all features"). _(superseded — see 002-auth0-user-auth)_: `auth0Id` field has been removed; `id` stores the Auth0 `sub` value directly and is the sole PK.

- **Account**: Represents the account/profile details associated with a User. Key attributes: `id` (UUIDv4), `userId` (foreign key to User — stores the User's `id`, which is the Auth0 `sub` value), `subscriptionTier` (free | premium — default: free, managed by subscription feature), `createdAt` (ISO 8601), `updatedAt` (ISO 8601). The Account entity is created alongside the User during signup and deleted alongside the User during account deletion. It provides the extension point for subscription management (Sous Chef FR-040/FR-041) without overloading the User entity.
