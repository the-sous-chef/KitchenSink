...
| -------------- | -------------------------- | --------------------------- | -------------------------------------------------------------------------------------------- |
| `id`           | `VARCHAR(255) COLLATE "C"` | `PRIMARY KEY`               | Auth0 subject claim value (e.g., `auth0\|abc123`). Canonical user identifier across all systems. |
...
> **Superseded columns** _(superseded — see 002-auth0-user-auth)_: `id UUID PRIMARY KEY DEFAULT gen_random_uuid()` and `auth0_id TEXT UNIQUE NOT NULL` have been removed. `users.id` (VARCHAR, Auth0 sub value) is now the sole PK and canonical identifier. No `app_metadata.userId` writeback is required.
...
| `user_id`           | `VARCHAR(255)` | `UNIQUE NOT NULL REFERENCES users(id) ON DELETE CASCADE` | 1:1 FK to User (`UNIQUE` enforces one account per user).                                             |
...
| `user_id`      | `VARCHAR(255)` | `UNIQUE NOT NULL REFERENCES users(id) ON DELETE CASCADE` | 1:1 FK to User.                                                       |
...
Auth0 signup
  ...
  +--> existing post-registration Action/Trigger chain
          ...
          +--> post-registration Lambda
                - UPSERT users ON CONFLICT (id) DO UPDATE
                - UPSERT accounts
                - UPSERT profiles
                - No app_metadata writeback required (id is the PK)

Authenticated API request
  |
  +--> API Gateway REQUEST authorizer Lambda
        - Verify JWT / claims / status
        - Inject context.id (from JWT sub claim)
  |
  +--> Identity Service (ECS) uses context.id for DB operations
...
Nightly reconciliation
  ...
  +--> EventBridge Scheduler triggers reconciliation Lambda
        - Diff Auth0 users vs DB users by id
...
| ------------------------------------------------ | ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `users.id` (VARCHAR PK)                          | REQ-013, REQ-015, FR-013, FR-015                           | Auth0 `sub` claim stored as `id`; no UUID writeback to `app_metadata` required                        |
| Atomic create: `users + accounts + profiles`     | REQ-014, FR-014                                            | Signup bootstrap path; UPSERT ON CONFLICT (id)                                                        |
...
| Reconciliation repair writes                     | REQ-017, FR-017                                            | Backfills missing records from Auth0; keyed by `id`                                                  |
...
1. `users.id` is the only canonical user identifier across Sous Chef systems. It stores the Auth0 `sub` claim value (e.g., `auth0|abc123`).
