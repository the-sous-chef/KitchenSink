# User Journeys: Auth0 User Authentication
...
**Scenario**: Avery opens the app (mobile) and the website (web), signs up via Auth0, and is provisioned into Sous Chef with a canonical user identity.
...
sequenceDiagram
    ...
    A0->>WH: Trigger post-registration action
...
sequenceDiagram
    ...
    API->>A0: Delete Auth0 user
    alt Auth0 delete succeeds
        API-->>APP: Complete deletion + logout
    else Auth0 delete fails
...
### 2. Async Auth0 Deletion Retry

- Trigger: local DB deletion successful, Auth0 Management API failure.
