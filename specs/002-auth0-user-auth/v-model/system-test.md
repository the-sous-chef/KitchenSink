# System Test Plan: Auth0 User Authentication
...
This document defines the System Test Plan for Auth0 User Authentication. Every system component
...
**Description**: Verifies that the Next.js web client initiates Auth0 Authorization Code Flow with PKCE and issues a 302 redirect to the Auth0 login page when an unauthenticated request hits a protected route.
...
- **System Scenario: STS-001-A1**
    ...
    - **Then** the response is HTTP 302 with `Location` header pointing to the Auth0 `/authorize` endpoint, including `response_type=code`, `code_challenge`, `code_challenge_method=S256`, and `state` parameters

- **System Scenario: STS-001-A2**
    - **Given** the Auth0 `/authorize` endpoint returns an error response (e.g., `access_denied`)
...
**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that the Auth0 callback handler exchanges the authorization code for tokens and sets a secure httpOnly session cookie.

- **System Scenario: STS-001-B1**
    - **Given** Auth0 has issued a valid authorization code and state parameter
...
- **System Scenario: STS-001-C1**
    - **Given** a successful Auth0 callback has been processed
...
**Description**: Verifies that the Expo mobile client presents the Auth0 authentication screen automatically when `expo-secure-store` contains no valid session tokens.
...
- **System Scenario: STS-002-A1**
    ...
    - **Then** the `react-native-auth0` SDK's `authorize()` method is invoked and the Auth0 login UI is presented before any app content renders
...
- **System Scenario: STS-002-B1**
    - **Given** the Auth0 authorization code flow completes successfully on mobile
...
**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that the Auth0 social login connection for Google is surfaced on both platforms and that the actual OAuth flow is delegated to Auth0.

- **System Scenario: STS-003-A1**
    - **Given** the Auth0 tenant has the Google social connection enabled
    - **When** the web client initiates login with `connection=google-oauth2` parameter in the `/authorize` request
    - **Then** Auth0 redirects to Google's OAuth consent screen; the Sous Chef backend receives no Google credentials directly

- **System Scenario: STS-003-A2**
    - **Given** the Auth0 tenant has the Google social connection enabled
    - **When** the mobile client calls `authorize({ connection: 'google-oauth2' })` via `react-native-auth0`
    - **Then** the Google OAuth consent screen is presented within the in-app browser; tokens are returned to the mobile client via the Auth0 callback

#### Test Case: STP-003-B (Social Login Unavailability — Fault Propagation)

**Technique**: Fault Injection
**Target View**: Dependency View
**Description**: Verifies behavior when the Auth0 social connection is disabled or returns an error.

- **System Scenario: STS-003-B1**
    - **Given** the Google social connection is disabled in the Auth0 tenant
    - **When** a login attempt is made with `connection=google-oauth2`
    - **Then** Auth0 returns an `access_denied` error; the client displays an error state and does not crash
...
- **System Scenario: STS-004-A1**
    ...
    - **Then** the refresh handler calls the Auth0 token endpoint with `grant_type=refresh_token`, receives a new access token, stores it in secure storage, and retries the original API call — all without presenting an auth screen
...
- **System Scenario: STS-004-B1**
    - **Given** the refresh token has been revoked in Auth0 (e.g., via logout on another device)
    - **When** the token refresh handler calls the Auth0 token endpoint with the revoked refresh token
    - **Then** Auth0 returns HTTP 400 with `error=invalid_grant`; the handler clears all tokens from secure storage and triggers the platform-appropriate re-authentication flow (auth screen on mobile, login redirect on web)

- **System Scenario: STS-004-B2**
    - **Given** the refresh token has expired per Auth0 configuration
...
### Component Verification: SYS-005 (Post-Registration Auth0 Action)
...
**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that the post-registration Auth0 Action generates a UUIDv4 user ID and writes it to `app_metadata` on the Auth0 user object.

- **System Scenario: STS-005-A1**
    - **Given** a new user completes Auth0 registration (email/password or social)
    - **When** the post-registration Action fires with the Auth0 user object
    - **Then** the Action generates a UUIDv4 ID, calls `api.user.setAppMetadata({ userId: <uuid> })`, and the Auth0 user's `app_metadata.userId` is set to the generated UUID
...
- **System Scenario: STS-005-B1**
    - **Given** `app_metadata.userId` has been set on the Auth0 user
    - **When** the Action calls the provisioning endpoint
    - **Then** the HTTP POST to the provisioning service includes `{ auth0Id, userId, email }` in the request body and the service responds HTTP 201; the Action returns HTTP 200 to Auth0
...
- **System Scenario: STS-005-C1**
    ...
    - **When** the Action executes its retry loop
    - **Then** the Action retries with delays of ~1s and ~2s (exponential backoff), succeeds on the third attempt, and returns HTTP 200 to Auth0
...
- **System Scenario: STS-005-C2**
    ...
    - **Then** the Action logs the failure with structured context and returns HTTP 500 to Auth0 (causing Auth0 to block the registration flow)
...
- **System Scenario: STS-006-A1**
    - **Given** the provisioning service receives `POST /internal/provision-user` with `{ auth0Id, userId, email }`
    - **When** the handler executes the database transaction
    - **Then** a `users` row is inserted with `id = userId`, `auth0_id = auth0Id`, `email = email`; an `accounts` row is inserted with `user_id = userId`; the response is HTTP 201 with `{ userId }`

#### Test Case: STP-006-B (Idempotency — Duplicate Provisioning Request)

**Technique**: Equivalence Partitioning
**Target View**: Data Design View
**Description**: Verifies that a duplicate provisioning call (same `auth0Id`) does not create duplicate records.

- **System Scenario: STS-006-B1**
    - **Given** a User record with `auth0_id = auth0Id` already exists in the database
    - **When** the provisioning service receives a second `POST /internal/provision-user` with the same `auth0Id`
...
**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that the reconciliation job identifies Auth0 users without Sous Chef database records and creates the missing User/Account records.

- **System Scenario: STS-007-A1**
    - **Given** Auth0 contains 3 users and the Sous Chef database contains records for only 2 of them
...
- **System Scenario: STS-010-A1**
    ...
    - **Then** the handler proceeds with deletion: removes User and Account records (cascading to user-owned data), calls Auth0 Management API to delete the Auth0 user, and returns HTTP 204
...
- **System Scenario: STS-010-B1**
    ...
    - **Then** the `users` row, `accounts` row, and all rows in related tables (recipes, meal_plans, etc.) with `user_id` FK are deleted within the same transaction; the Auth0 user is deleted after the DB transaction commits
...
#### Test Case: STP-011-A (Password Reset Delegation to Auth0)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that the password reset flow is fully delegated to Auth0 and the Sous Chef backend never handles password data.
...
- **System Scenario: STS-011-A1**
    ...
    - **Then** the client redirects to Auth0's hosted password reset page; no password data is sent to or processed by any Sous Chef backend endpoint

- **System Scenario: STS-011-A2**
    - **Given** Auth0's password reset flow completes successfully
...
#### Test Case: STP-012-A (MFA Enrollment via Auth0 — TOTP)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that MFA enrollment is surfaced from account settings and delegated to Auth0's MFA enrollment flow.
...
- **System Scenario: STS-012-A1**
    ...
    - **Then** the client redirects to Auth0's MFA enrollment UI; the Sous Chef backend does not receive or store TOTP secrets

#### Test Case: STP-012-B (MFA Enforcement on Subsequent Login)

**Technique**: Equivalence Partitioning
**Target View**: Decomposition View
**Description**: Verifies that Auth0 enforces the second factor on login after MFA enrollment.

- **System Scenario: STS-012-B1**
    - **Given** a user has completed MFA enrollment in Auth0
    - **When** the user initiates a new login via the Authorization Code Flow
    - **Then** Auth0 challenges the user for the TOTP second factor before issuing tokens; the Sous Chef client receives tokens only after successful MFA verification
...
**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that linking a social provider via Auth0 Management API does not create new User/Account records and preserves the canonical Sous Chef user ID.
...
- **System Scenario: STS-013-A1**
    ...
    - **Then** the handler calls Auth0 Management API to link the social identity to the existing Auth0 user; no new `users` or `accounts` rows are created; `app_metadata.userId` remains unchanged
...
- **System Scenario: STS-013-B1**
    ...
    - **Then** the handler returns HTTP 400 indicating the last provider cannot be unlinked; the Auth0 identity is not modified
...
**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that impersonation sessions are established via Auth0 token exchange and flagged in the token claims.
...
- **System Scenario: STS-014-A1**
    ...
    - **Then** the system exchanges for an impersonation token via Auth0; the resulting token contains a claim indicating impersonation (e.g., `impersonating: true`, `impersonated_user_id: <userId>`); the audit log records the impersonation event
...
- **System Scenario: STS-015-A1**
    - **Given** a valid, unexpired JWT signed by the Auth0 tenant's JWKS, with correct `aud` and `iss` claims
...
#### Test Case: STP-016-A (Suspend User — Auth0 Block and DB Status Update)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that suspending a user blocks them in Auth0 and sets `status = suspended` in the database.
...
- **System Scenario: STS-016-A1**
    ...
    - **Then** the handler calls Auth0 Management API to block the user (`blocked: true`), updates `users.status = 'suspended'` in the database, and returns HTTP 200; subsequent login attempts by the user are rejected by Auth0

#### Test Case: STP-016-B (Reactivate User — Auth0 Unblock and DB Status Update)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that reactivating a user unblocks them in Auth0 and sets `status = active` in the database.
...
- **System Scenario: STS-016-B1**
    ...
    - **Then** the handler calls Auth0 Management API to unblock the user (`blocked: false`), updates `users.status = 'active'` in the database, and returns HTTP 200; the user can subsequently authenticate successfully
...
| STP-005-A | SYS-005 Post-Registration Action   | Interface Contract Testing | 1         |
| STP-005-B | SYS-005 Post-Registration Action   | Interface Contract Testing | 1         |
| STP-005-C | SYS-005 Post-Registration Action   | Fault Injection            | 2         |
