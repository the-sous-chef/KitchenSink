# Integration Test Plan: Auth0 User Authentication
...
#### Test Case: ITP-001-A (Auth0 Callback Code Exchange → Session Cookie Handshake)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that ARCH-001 correctly receives the OAuth authorization code from Auth0 and delivers a valid session cookie to ARCH-003, conforming to the `{code, state}` → `{session cookie}` contract.

- **Integration Scenario: ITS-001-A1**
    - **Given** Auth0 delivers a `GET /api/auth/callback?code=<valid_code>&state=<valid_state>` request to ARCH-001
    - **When** ARCH-001 exchanges the code with Auth0's `/oauth/token` endpoint and receives `{access_token, refresh_token, id_token}`
...
- **Integration Scenario: ITS-001-A2**
    - **Given** Auth0 delivers a callback with a mismatched `state` parameter to ARCH-001
...
#### Test Case: ITP-001-B (Auth0 Token Exchange Failure Propagation)

**Technique**: Interface Fault Injection
**Target View**: Interface View + Process View
**Description**: Verifies that ARCH-001 correctly propagates Auth0 token exchange failures to the caller without leaking internal state.

- **Integration Scenario: ITS-001-B1**
    - **Given** Auth0's `/oauth/token` endpoint returns HTTP 400 `{error: "invalid_grant"}` to ARCH-001
    - **When** ARCH-001 receives the error response from Auth0
...
#### Test Case: ITP-004-A (Secure Store Miss → Auth0 Login Trigger)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies the handshake between ARCH-004 and ARCH-005: on app launch, ARCH-004 calls ARCH-005's `getToken()` and triggers the Auth0 login flow when no token is found.
...
- **Integration Scenario: ITS-004-A1**
    ...
    - **Then** ARCH-004 triggers the Auth0 login screen via `react-native-auth0` and the React context state is set to `{isAuthenticated: false}`
...
- **Integration Scenario: ITS-004-A2**
    ...
    - **Then** ARCH-004 sets React context state to `{isAuthenticated: true}` without triggering the Auth0 login screen
...
**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies the full data flow: ARCH-006 receives the deep-link authorization code, exchanges it with Auth0, stores tokens via ARCH-005, and signals ARCH-004.
...
- **Integration Scenario: ITS-006-A1**
    ...
    - **When** ARCH-006 exchanges the code with Auth0's `/oauth/token` and receives `{access_token, refresh_token}`
...
**Technique**: Interface Fault Injection
**Target View**: Interface View + Process View
**Description**: Verifies that ARCH-006 does not write to ARCH-005 when Auth0 token exchange fails.

- **Integration Scenario: ITS-006-B1**
    - **Given** Auth0's `/oauth/token` returns HTTP 400 to ARCH-006
...
#### Test Case: ITP-007-A (Social Provider Config → Auth0 Tenant Contract)
...
- **Integration Scenario: ITS-007-A1**
    - **Given** ARCH-007 has configured the Google social connection in the Auth0 tenant
    - **When** ARCH-001 initiates the Auth0 login redirect
    - **Then** the Auth0 authorization URL includes the `connection=google-oauth2` parameter and the Auth0 login page presents the Google login option

- **Integration Scenario: ITS-007-A2**
    - **Given** ARCH-007 has configured the Google social connection
    - **When** ARCH-004 opens the Auth0 login screen on mobile
    - **Then** the Auth0 login screen presents the Google login option alongside email/password
...
**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies the data flow: ARCH-008 receives an expired token from ARCH-003, exchanges it with Auth0, and delivers the refreshed token set back to ARCH-003 for cookie update.

- **Integration Scenario: ITS-008-A1**
    - **Given** ARCH-002 delegates to ARCH-008 with an expired access token from ARCH-003
    - **When** ARCH-008 calls Auth0's `/oauth/token` with `grant_type=refresh_token` and receives a new `{access_token, refresh_token}`
...
**Technique**: Interface Fault Injection
**Target View**: Interface View + Process View
**Description**: Verifies that ARCH-008 correctly signals ARCH-002 to redirect to login when Auth0 returns `invalid_grant`.

- **Integration Scenario: ITS-008-B1**
    - **Given** ARCH-008 calls Auth0's `/oauth/token` with a revoked refresh token
    - **When** Auth0 returns HTTP 400 `{error: "invalid_grant"}`
...
**Description**: Verifies that concurrent requests with the same expired token do not trigger multiple simultaneous refresh calls to Auth0.
...
- **Integration Scenario: ITS-008-C1**
    ...
    - **Then** only one refresh call is made to Auth0 (the second waits for the first), and both requests receive the same refreshed session cookie from ARCH-003
...
**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies that ARCH-009 reads the refresh token from ARCH-005, exchanges it with Auth0, and writes the new token set back to ARCH-005.

- **Integration Scenario: ITS-009-A1**
    - **Given** ARCH-004 detects an expired access token and delegates to ARCH-009
    - **When** ARCH-009 reads the refresh token from ARCH-005's `getToken()` and calls Auth0's `/oauth/token`
...
- **Integration Scenario: ITS-009-B1**
    - **Given** ARCH-009 calls Auth0's `/oauth/token` with a revoked refresh token
    - **When** Auth0 returns HTTP 400 `{error: "invalid_grant"}`
    - **Then** ARCH-009 calls ARCH-005's `clearTokens()` and signals ARCH-004 to trigger the Auth0 login screen

---

### Module Verification: ARCH-010 (Post-Registration Auth0 Action)

**Parent System Components**: SYS-005

#### Test Case: ITP-010-A (Auth0 Registration Event → UUIDv4 → Provisioning Lambda Call)

**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies the data flow from Auth0's `post-user-registration` event through ARCH-010 to ARCH-011, including the `app_metadata.userId` write.

- **Integration Scenario: ITS-010-A1**
    - **Given** Auth0 fires a `post-user-registration` event to ARCH-010 with `{user: {user_id, email}}`
    - **When** ARCH-010 generates a UUIDv4 `userId`, writes it to Auth0 `app_metadata.userId` via the Management API, and calls ARCH-011 with `{auth0Id, userId, email}`
...
- **Integration Scenario: ITS-010-B1**
    ...
    - **Then** ARCH-010 reports an action failure to Auth0 and the error is captured by ARCH-029 (Sentry)
...
**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that ARCH-011 correctly receives `{auth0Id, userId, email}` from ARCH-010 and writes both `users` and `accounts` records in a single PostgreSQL transaction.

- **Integration Scenario: ITS-011-A1**
    - **Given** ARCH-010 calls ARCH-011's `POST /internal/provision-user` with `{auth0Id: string, userId: UUIDv4, email: string}`
...
- **Integration Scenario: ITS-011-C1**
    - **Given** ARCH-010 calls ARCH-011 twice with the same `{auth0Id, userId, email}` (e.g., due to retry)
...
#### Test Case: ITP-012-A (Auth0 User List → DB Comparison → Provisioning Delegation)

**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies that ARCH-012 correctly fetches the Auth0 user list, compares it with the Sous Chef DB, and delegates missing records to ARCH-011.

- **Integration Scenario: ITS-012-A1**
    - **Given** Auth0 Management API returns a user list containing `user_A` and `user_B`, but only `user_A` exists in the Sous Chef DB
    - **When** ARCH-012 compares the Auth0 list with the DB
    - **Then** ARCH-012 calls ARCH-011's `POST /internal/provision-user` for `user_B` only

#### Test Case: ITP-012-B (Auth0 Management API Failure → Graceful Abort)

**Technique**: Interface Fault Injection
**Target View**: Interface View + Process View
**Description**: Verifies that ARCH-012 aborts reconciliation and logs the error when the Auth0 Management API is unavailable.

- **Integration Scenario: ITS-012-B1**
    - **Given** the Auth0 Management API returns HTTP 503 to ARCH-012
...
#### Test Case: ITP-017-A (Delete Request → DB Cascade → Auth0 Deletion)

**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies the full deletion data flow: ARCH-017 cascades DB deletion and then calls Auth0 Management API to delete the user.

- **Integration Scenario: ITS-017-A1**
    - **Given** ARCH-024 passes `{userId: UUIDv4, auth0Id: string}` to ARCH-017 via request context
    - **When** ARCH-017 executes `DELETE FROM accounts WHERE user_id = userId` (cascade) and `DELETE FROM users WHERE id = userId`
    - **Then** ARCH-017 calls Auth0 Management API `DELETE /api/v2/users/{auth0Id}` and returns HTTP 200 on success

#### Test Case: ITP-017-B (Auth0 Deletion Failure → DB Rollback)

**Technique**: Interface Fault Injection
**Target View**: Interface View + Process View
**Description**: Verifies that ARCH-017 rolls back DB changes if the Auth0 Management API deletion fails.

- **Integration Scenario: ITS-017-B1**
    - **Given** ARCH-017 has successfully deleted DB records but Auth0 Management API returns HTTP 500
    - **When** ARCH-017 receives the Auth0 error
...
#### Test Case: ITP-018-A (Password Reset Redirect → Auth0 Hosted Page Contract)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that ARCH-018 produces a correctly formatted redirect to Auth0's hosted password reset page.

- **Integration Scenario: ITS-018-A1**
    - **Given** ARCH-018 is rendered on the login screen
    - **When** the "Forgot Password" link is activated
    - **Then** ARCH-018 redirects to Auth0's hosted password reset URL with the correct `client_id` and `connection` parameters, and no backend module is invoked
...
#### Test Case: ITP-019-A (MFA Enrollment Redirect → Auth0 Management API Link Contract)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that ARCH-019 correctly constructs the Auth0 MFA enrollment redirect URL using the Management API link.
...
- **Integration Scenario: ITS-019-A1**
    ...
    - **Then** ARCH-019 redirects to the Auth0 MFA enrollment page URL obtained via the Management API, with the correct `user_id` parameter
...
#### Test Case: ITP-020-A (Link Request → Auth0 Management API → DB Update)

**Technique**: Interface Contract Testing
**Target View**: Interface View
**Description**: Verifies that ARCH-020 correctly calls Auth0 Management API to link a social provider and reflects the change.
...
- **Integration Scenario: ITS-020-A1**
    ...
    - **When** ARCH-024 authorizes the request and ARCH-020 calls Auth0 Management API `POST /api/v2/users/{auth0Id}/identities`
    - **Then** Auth0 returns the updated identities array and ARCH-020 returns HTTP 200 with the linked provider list
...
- **Integration Scenario: ITS-020-B1**
    ...
    - **Then** ARCH-020 returns HTTP 400 `{error: "LastProviderError", message: "At least one provider must remain"}` without calling Auth0 Management API
...
#### Test Case: ITP-022-A (Impersonation Request → Auth0 Token Exchange → Claims Injection)
...
- **Integration Scenario: ITS-022-A1**
    ...
    - **When** ARCH-022 calls Auth0's token exchange endpoint
...
- **Integration Scenario: ITS-022-B1**
    ...
    - **Then** ARCH-022 returns HTTP 403 without calling Auth0's token exchange endpoint
...
**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies the full data flow: ARCH-024 fetches JWKS from Auth0, validates the JWT, extracts `app_metadata.userId`, delegates to ARCH-025, and returns an IAM Allow policy.

- **Integration Scenario: ITS-024-A1**
    - **Given** API Gateway invokes ARCH-024 with a valid Bearer JWT
    - **When** ARCH-024 fetches the JWKS from Auth0 (or uses the cached set), validates signature/expiry/audience/issuer, and extracts `app_metadata.userId`
...
**Technique**: Concurrency & Race Condition Testing
**Target View**: Process View
**Description**: Verifies that concurrent Lambda invocations reuse the in-memory JWKS cache rather than making redundant Auth0 JWKS fetches.
...
- **Integration Scenario: ITS-024-C1**
    ...
    - **Then** both requests use the cached JWKS without making additional calls to Auth0's JWKS endpoint
...
- **Integration Scenario: ITS-025-A1**
    - **Given** ARCH-024 calls ARCH-025 with a `userId` that is not suspended in Auth0 or the Sous Chef DB
    - **When** ARCH-025 checks the `blocked` flag in Auth0 and the `status` field in the DB
...
- **Integration Scenario: ITS-025-B1**
    - **Given** ARCH-024 calls ARCH-025 with a `userId` that has `blocked: true` in Auth0
...
#### Test Case: ITP-026-A (Suspend Request → Auth0 Block + DB Status Update)

**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies that ARCH-026 correctly calls Auth0 Management API to block the user and updates the Sous Chef DB `status` field.

- **Integration Scenario: ITS-026-A1**
    - **Given** an admin sends `POST /admin/users/{id}/suspend` with a valid admin JWT
    - **When** ARCH-024 authorizes the request and ARCH-026 calls Auth0 Management API `PATCH /api/v2/users/{auth0Id}` with `{blocked: true}`
    - **Then** Auth0 returns HTTP 200, ARCH-026 updates `users.status = "suspended"` in PostgreSQL, and returns HTTP 200

#### Test Case: ITP-026-B (Reactivation → Auth0 Unblock + DB Status Update)

**Technique**: Data Flow Testing
**Target View**: Data Flow View
**Description**: Verifies that ARCH-026 correctly unblocks the user in Auth0 and updates the DB status to active.

- **Integration Scenario: ITS-026-B1**
    - **Given** an admin sends `POST /admin/users/{id}/reactivate` with a valid admin JWT
    - **When** ARCH-026 calls Auth0 Management API with `{blocked: false}`
    - **Then** Auth0 returns HTTP 200, ARCH-026 updates `users.status = "active"` in PostgreSQL, and returns HTTP 200
...
- **Integration Scenario: ITS-032-A1**
    ...
    - **Then** the type guard returns `true` and ARCH-002 redirects to the Auth0 login page

- **Integration Scenario: ITS-032-A2**
    - **Given** ARCH-017 throws an `AccountDeletionFailedError` (from ARCH-032) when Auth0 deletion fails
...
| --------- | ------------------------------ | ---------------------------------------------------- | ------------------------------------------------------------------- |
| ITP-001-A | Auth0 `/oauth/token`           | HTTP stub returning valid token response             | Isolate ARCH-001↔ARCH-003 boundary from Auth0 availability          |
| ITP-001-B | Auth0 `/oauth/token`           | HTTP stub returning 400 `invalid_grant`              | Test error propagation without live Auth0                           |
...
| ITP-006-B | Auth0 `/oauth/token`           | HTTP stub returning 400                              | Verify ARCH-006 does not write to ARCH-005 on failure               |
| ITP-008-C | Auth0 `/oauth/token`           | HTTP stub with 200ms delay                           | Simulate concurrent refresh race; verify single Auth0 call          |
| ITP-010-A | Auth0 Management API           | HTTP stub for `PATCH /api/v2/users/{id}`             | Isolate ARCH-010 from live Auth0 Management API                     |
| ITP-011-B | PostgreSQL (RDS)               | Fault-injecting DB proxy (first call fails)          | Simulate transient DB failure for retry verification                |
| ITP-011-C | PostgreSQL (RDS)               | Real DB with unique constraint on `auth0_id`         | Verify idempotency via DB constraint                                |
| ITP-012-A | Auth0 Management API + DB      | HTTP stub for user list; seeded test DB              | Control reconciliation inputs precisely                             |
| ITP-017-B | Auth0 Management API           | HTTP stub returning 500 on DELETE                    | Verify DB rollback on Auth0 deletion failure                        |
| ITP-022-B | Auth0 token exchange           | Stub not called; verify via spy                      | Confirm Auth0 is not called for unauthorized impersonation requests |
| ITP-024-C | Auth0 JWKS endpoint            | Spy on JWKS fetch; warm Lambda instance              | Verify cache reuse across concurrent invocations                    |
| ITP-026-A | Auth0 Management API + DB      | HTTP stub for PATCH; real test DB                    | Verify both Auth0 block and DB status update                        |
