...
**Dual-runtime note (added 2026-05-14, T18 doc-sync):** ARCH-010, ARCH-011, ARCH-012, ARCH-024, ARCH-025 run as raw **AWS Lambda** functions (Node 22, `packages/services/identity-webhooks/`). ARCH-015, ARCH-017, ARCH-020, ARCH-022, ARCH-023, ARCH-026 run inside the **NestJS 11 ECS service** (Node 24, `packages/services/identity/`). ARCH-027/028/029 (observability) are used by both runtimes. ARCH-030 (CDK Auth Stack) provisions ECS and Lambda resources via CDK v2 + Serverless Framework respectively.
...
| ARCH-015 | Account Edit API Handler             | **[ECS — NestJS]** NestJS controller/service handling `PATCH /v1/users/me` and `PATCH /v1/users/me/profile`. Validates input via `class-validator` DTOs, persists display name and avatar changes to PostgreSQL. Receives identity from `AuthorizerContext` header.                                                 | SYS-009                  | Service   |
...
| ARCH-017 | Account Deletion API Handler         | **[ECS — NestJS]** NestJS controller/service handling `DELETE /v1/users/me`. Deletes User/Account records (cascading to user-owned data) from PostgreSQL, enqueues Auth0 deletion via SQS (`UserDeletionQueueMessage`), then returns 204. Auth0 deletion is completed asynchronously by the deletion worker Lambda. | SYS-010                  | Service   |
...
| ARCH-020 | Social Account Linking API Handler   | **[ECS — NestJS]** NestJS controller/service handling `POST /v1/users/me/social-link` and `DELETE /v1/users/me/social-link`. Calls Auth0 Management API to link/unlink social providers. Validates at least one provider remains.                                                                                   | SYS-013                  | Service   |
...
| ARCH-022 | Impersonation Token Exchange Service | **[ECS — NestJS]** NestJS admin service that issues impersonation tokens via Auth0 token exchange for authorized personnel. Injects impersonation flag and impersonator identity into token claims.                                                                                                                 | SYS-014                  | Service   |
...
| ARCH-026 | User Suspension API Handler          | **[ECS — NestJS]** NestJS admin controller/service handling `POST /v1/admin/users/:id/suspend` and `POST /v1/admin/users/:id/unsuspend`. Calls Auth0 Management API to block/unblock and updates Sous Chef DB `status`.                                                                                             | SYS-016                  | Service   |
| ARCH-027 | Structured Logger                    | [CROSS-CUTTING; rationale: shared infrastructure supports multiple SYS components] — Wraps `@aws-lambda-powertools/logger` (Lambda runtime) and NestJS logger (ECS runtime) to emit JSON-structured logs with correlation IDs and ISO 8601 timestamps. Used by all Lambda functions and the ECS identity service.   | SYS-017                  | Utility   |
| ARCH-028 | CloudWatch Metrics Emitter           | [CROSS-CUTTING; rationale: shared infrastructure supports multiple SYS components] — Emits custom CloudWatch metrics for auth flow events (login success/failure, token refresh, signup, deletion, reconciliation). Used by all auth backend services in both Lambda and ECS runtimes.                              | SYS-017                  | Utility   |
| ARCH-029 | Sentry Integration Wrapper           | [CROSS-CUTTING; rationale: shared infrastructure supports multiple SYS components] — Wraps `@sentry/aws-serverless` for Lambda error capture and `@sentry/node` for ECS error capture; `@sentry/react`/`@sentry/react-native` for client-side breadcrumbs and error events.                                         | SYS-017                  | Utility   |
