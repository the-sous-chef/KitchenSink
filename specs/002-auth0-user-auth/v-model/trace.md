# V-Model Traceability Matrix: Auth0 User Authentication
...
| -------------------------- | ------------------------------------------------------ | ---------- | ---------- | --------------------------------------------------------------- |
| Requirements Specification | `specs/002-auth0-user-auth/v-model/requirements.md`    | 2026-05-09 | Draft      | 44 FR + 17 NF + 10 IF + 7 CN + 1 CN-008 = 79 total requirements |
| Acceptance Test Plan       | `specs/002-auth0-user-auth/v-model/acceptance-plan.md` | 2026-05-09 | Draft      | AT cases for all 44 FR + selected NF/IF/CN                      |
| Unit Test Plan             | `specs/002-auth0-user-auth/v-model/unit-test.md`       | 2026-05-09 | Draft      | 6 MODs, 8 UTP cases, 30 UTS scenarios                           |
| System Design              | `specs/002-auth0-user-auth/v-model/system-design.md`   | —          | Referenced | ARCH modules (ARCH-001 through ARCH-006+)                       |
| Module Design              | `specs/002-auth0-user-auth/v-model/module-design.md`   | —          | Referenced | MOD-001 through MOD-006                                         |
...
| REQ-003 | Web redirects unauthenticated users to Auth0 login                              | P1       | AT-003-A           | Protected routes redirect; public routes unaffected              | Scenario-Based Testing   | ⬜     |
| REQ-004 | Social login (Google) supported                                                 | P1       | AT-004-A           | Google OAuth completes; cancel handled                           | Scenario-Based Testing   | ⬜     |
| REQ-005 | Auth0 callback exchanges code for tokens                                        | P1       | AT-005-A           | Token exchange completes; tampered state rejected                | Scenario-Based Testing   | ⬜     |
...
| REQ-011 | Logout revokes refresh token in Auth0                                           | P1       | AT-011-A           | Revoked refresh token rejected by Auth0                          | Scenario-Based Testing   | ⬜     |
| REQ-012 | Post-logout navigation to auth screen / login page                              | P1       | AT-012-A           | Web → Auth0 login page; Mobile → Auth0 login UI                  | Scenario-Based Testing   | ⬜     |
...
| REQ-015 | User ID stored in Auth0 app_metadata                                            | P1       | AT-015-A           | UUIDv4 present as custom claim in access token                   | Scenario-Based Testing   | ⬜     |
...
| REQ-021 | Email field read-only on account edit page                                      | P2       | AT-021-A           | Email field is read-only with Auth0 redirect note                | Scenario-Based Testing   | ⬜     |
...
| REQ-024 | Confirmed deletion removes User/Account from DB and Auth0                       | P2       | AT-024-A           | Records absent from DB and Auth0 after deletion                  | Scenario-Based Testing   | ⬜     |
...
| REQ-027 | Password reset accessible from login screen                                     | P2       | AT-027-A           | "Forgot Password" initiates Auth0 reset flow                     | Scenario-Based Testing   | ⬜     |
...
| REQ-041 | Suspension blocks user in Auth0 and sets DB status                              | P2       | AT-041-A           | User blocked in Auth0; DB status = suspended                     | Scenario-Based Testing   | ⬜     |
...
| REQ-ID     | Requirement (Summary)                                                   | Priority | ATP-ID                         | Acceptance Test (Summary)                                       | Verification Method        | Status |
| ---------- | ----------------------------------------------------------------------- | -------- | ------------------------------ | --------------------------------------------------------------- | -------------------------- | ------ |
| REQ-IF-001 | Integrate with Auth0 via `@auth0/nextjs-auth0` v4.x (web)               | P1       | _(Inspection — no AT defined)_ | —                                                               | Inspection                 | ⬜     |
| REQ-IF-002 | Integrate with Auth0 via `react-native-auth0` v5.5 (mobile)             | P1       | _(Inspection — no AT defined)_ | —                                                               | Inspection                 | ⬜     |
...
| REQ-IF-010 | Reconciliation endpoint accepts Auth0 user list                         | P1       | AT-IF-010-A                    | Missing records created; repair summary returned                | Scenario-Based Testing     | ⬜     |
...
| REQ-CN-003 | UUIDv4 is canonical user identifier; Auth0 sub for Auth0 calls only | P1       | AT-CN-003-A                    | Auth0 sub not used as primary key in DB or app logic       | Inspection          | ⬜     |
...
| REQ-CN-008 | All workspaces extend shared armoury configs                        | P1       | _(Inspection — no AT defined)_ | —                                                          | Inspection          | ⬜     |
...
| AT-003-A    | Protected web routes redirect to Auth0                    | REQ-003    | Web redirects unauthenticated users                      | Confirms redirect behavior for protected and public routes            |
...
| AT-005-A    | Callback handler completes token exchange                 | REQ-005    | Auth0 callback exchanges code for tokens                 | Confirms code exchange and tampered state rejection                   |
...
| AT-011-A    | Refresh token revoked server-side on logout               | REQ-011    | Logout revokes refresh token in Auth0                    | Confirms Auth0 rejects revoked refresh token                          |
| AT-012-A    | User lands on correct screen after logout                 | REQ-012    | Post-logout navigation                                   | Confirms web → Auth0 login page; mobile → Auth0 login UI              |
| AT-013-A    | New signup creates a User record                          | REQ-013    | Post-registration action creates User record             | Confirms UUIDv4 User record linked to Auth0 sub                       |
...
| AT-015-A    | UUIDv4 user ID present in access token claims             | REQ-015    | User ID stored in Auth0 app_metadata                     | Confirms custom claim in decoded access token                         |
| AT-016-A    | Transient DB failure does not orphan Auth0 user           | REQ-016    | Post-registration action retries on transient failure    | Confirms retry logic and failure logging                              |
...
| AT-021-A    | Email cannot be edited in the app                         | REQ-021    | Email field read-only                                    | Confirms read-only field with Auth0 redirect note                     |
...
| AT-024-A    | Account deletion removes all records                      | REQ-024    | Confirmed deletion removes records from DB and Auth0     | Confirms User/Account absent from both systems                        |
...
| AT-027-A    | "Forgot Password" link present and functional             | REQ-027    | Password reset accessible from login screen              | Confirms Auth0 reset flow initiated                                   |
...
| AT-029-A    | MFA enrollment entry point present                        | REQ-029    | MFA enrollment accessible from account settings          | Confirms MFA option visible and navigates to Auth0 flow               |
...
| AT-041-A    | Suspension applied in both systems                        | REQ-041    | Suspension blocks user in Auth0 and DB                   | Confirms Auth0 blocked + DB status = suspended                        |
...
| AT-IF-010-A | Reconciliation endpoint processes Auth0 user data         | REQ-IF-010 | Reconciliation endpoint accepts Auth0 user list          | Confirms missing records created and repair summary returned          |
...
| AT-CN-003-A | Auth0 sub not used as primary key in database             | REQ-CN-003 | UUIDv4 is canonical user identifier                      | Confirms sub used only for Auth0 API calls                            |
...
| ----------------------------------------------- | ---------------------------------- | --------------------------------------- | --------------------------------------- | ----------------------- | ---------------------------------------------------------------------------- |
| Web Auth Route Handler → Auth0 token endpoint   | REQ-001, REQ-005                   | MOD-001 ↔ Auth0 `/oauth/token`          | UTP-001-B (exchangeCodeForTokens)       | ⬜                      | Integration test needed: real Auth0 sandbox token exchange                   |
...
| Web JWT Decoder → jose library                  | REQ-039, REQ-040                   | MOD-004 ↔ jose                          | UTP-004-A (jose used directly)          | ⬜                      | Integration test needed: validate against real Auth0 JWKS                    |
...
| Mobile Auth0 Client → react-native-auth0 SDK    | REQ-001, REQ-007                   | MOD-006 ↔ react-native-auth0            | UTP-006-A (Auth0Client mocked)          | ⬜                      | Integration test needed: real SDK token refresh flow                         |
| Post-Registration Action → Sous Chef DB         | REQ-013, REQ-014, REQ-015, REQ-016 | Auth0 Action ↔ DB                       | No UTP (backend action)                 | ⬜                      | Integration test needed: action → DB write with retry                        |
| API Gateway Authorizer → Auth0 JWKS             | REQ-038, REQ-039, REQ-042          | Lambda Authorizer ↔ Auth0 JWKS endpoint | No UTP (authorizer module)              | ⬜                      | Integration test needed: authorizer validates real JWT                       |
| Account Deletion → Auth0 Management API         | REQ-024                            | Backend handler ↔ Auth0 Management API  | No UTP (deletion handler)               | ⬜                      | Integration test needed: deletion cascades to Auth0                          |
| Reconciliation Job → Auth0 Management API + DB  | REQ-017, REQ-IF-010                | Reconciliation ↔ Auth0 + DB             | No UTP (reconciliation module)          | ⬜                      | Integration test needed: reconciliation creates missing records              |
...
| UTP-006-A | MOD-006 Mobile Auth0 Client SDK Wrapper | Statement & Branch Coverage                            | REQ-001, REQ-007                            | 4         | ⬜     |
...
| HAZ-005 | Refresh token reuse after logout                       | High     | REQ-011                      | Refresh token revoked via Auth0 revocation endpoint on logout                                 | AT-011-A                          | ⬜     |
| HAZ-006 | Orphaned Auth0 user without Sous Chef record           | Medium   | REQ-016, REQ-017             | Retry with exponential backoff; reconciliation job as safety net                              | AT-016-A, AT-017-A                | ⬜     |
| HAZ-007 | Password exposure through Sous Chef backend            | Critical | REQ-028, REQ-CN-002          | All password management delegated to Auth0; no password fields in Sous Chef                   | AT-028-A, AT-CN-002-A             | ⬜     |
...
| HAZ-013 | Auth0 vendor lock-in via sub as primary identifier     | Low      | REQ-CN-003                   | UUIDv4 is canonical identifier; Auth0 sub used only for Auth0 API calls                       | AT-CN-003-A                       | ⬜     |
...
| HAZ-015 | Missing GDPR compliance on account deletion            | High     | REQ-024, REQ-025, REQ-CN-004 | Hard delete of User, Account, and all owned data from DB and Auth0; no soft-delete            | AT-024-A, AT-025-A                | ⬜     |
...
| MOD-006   | Mobile Auth0 Client SDK Wrapper | 1         | 4             | Statement & Branch, Strict Isolation                           |
...
| ------------------------------ | ---------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------- |
| Post-Registration Auth0 Action | No MOD or UTP defined for the Auth0 Action that creates User/Account records | High   | Define MOD-007 and UTP-007 covering retry logic (REQ-016), UUIDv4 generation (REQ-013, REQ-015), and DB write                |
...
| Account Deletion Handler       | No MOD or UTP defined for the deletion backend handler                       | Medium | Define MOD-009 and UTP-009 covering DB cascade (REQ-025) and Auth0 Management API call (REQ-024)                             |
| Reconciliation Job             | No MOD or UTP defined for the reconciliation mechanism                       | Medium | Define MOD-010 and UTP-010 covering Auth0 user list processing (REQ-017, REQ-IF-010)                                         |
