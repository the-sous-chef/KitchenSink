 ...
 *
 * Source spec: specs/002-auth0-user-auth/spec.md — Key Entities (AuthSession)
 */
 ...
 *
 * @see specs/002-auth0-user-auth/spec.md FR-006, FR-007, FR-008, FR-009
 */
export interface AuthSession {
  /**
   * Auth0 JWT access token. Included as `Authorization: Bearer <token>` on all API requests.
   * Expires at `expiresAt`.
   */
  readonly accessToken: string;
  /**
   * Opaque Auth0 refresh token. Used to silently refresh the access token when it expires.
   * Revoked in Auth0 on logout.
   */
   ...
   * Do NOT use `auth0Id` as an application identifier.
   */
  readonly userId: string;
  /**
   * Auth0 `sub` claim (e.g., `auth0|abc123`).
   * Used only for client-side Auth0 API calls. Not a Sous Chef application identifier.
   */
 ...
 * the `refreshToken` may be rotated (Auth0 refresh token rotation enabled).
 */
