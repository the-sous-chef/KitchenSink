...
| SYS-006 | Recipe Data Adapter     | Adapts the Recipe entity from feature 001-sous-chef-recipe-app into the Cooking Mode internal step model. Read-only; never mutates Recipe data.                                              | REQ-IF-001, REQ-CN-001                                        | Adapter   |
| SYS-007 | Auth Guard              | Verifies a valid authenticated session (via feature 002-auth0-user-auth) before allowing entry into Cooking Mode. Redirects unauthenticated users to the login flow.                         | REQ-IF-002                                                    | Service   |
