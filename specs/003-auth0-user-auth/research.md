...
When account deletion succeeds in the Sous Chef database but the Auth0 Management API call fails, the Auth0 deletion is queued for async retry rather than blocking the user.
...
DELETE /users/{id}
  ...
  └─► Auth0 delete attempt
        ├─ SUCCESS: Done
        └─ FAILURE: Enqueue { auth0Id, userId, attemptedAt, reason }
                      │
                      └─► SQS Standard Queue
                              │
                              ├─ visibilityTimeout = 6 × Lambda timeout
                              ├─ Lambda consumer (reads ApproximateReceiveCount)
                              │   ├─ Attempt Auth0 Management API delete
...
The reconciliation job detects Auth0 users without a corresponding Sous Chef database record and creates the missing records. This covers:

1. Post-registration action timeout during signup (Auth0 user created, DB write never happened)
2. Failed initial DB write that exhausted all retries

**Efficient diffing strategy (up to ~1M users)**:

```
1. Fetch all Auth0 user IDs (paginated, 100 per page via /api/v2/users?fields=user_id,app_metadata)
...
2. Fetch all active user IDs from our DB for the same 7-day window
   - SELECT id, auth0_id FROM users WHERE created_at > NOW() - INTERVAL '7 days'

3. Set difference: Auth0 users NOT IN our DB users (by auth0_id)
   → These are orphaned Auth0 users → create User + Account records + update app_metadata

4. Set difference: our DB users NOT IN Auth0 (by auth0_id)
   → These should not exist (Auth0 is authoritative) → log for manual review, do not auto-delete
```

**Auth0 Management API pagination**: The `/api/v2/users` endpoint returns 100 users per page. Use `page` and `per_page` parameters. For large tenants, prefer checkpoint-based reconciliation (run every hour, check last 24h) over full scans.

**Rate limit handling**: Auth0 Management API has per-tenant rate limits (varies by plan). Use `p-limit` or `bottleneck` to cap concurrent requests. On 429, honor the `Retry-After` header and use the EventBridge Scheduler `maximumRetryAttempts` for Lambda-level retries.
...
| **Auth0 post-registration action timeout is 20s**                      | If the action times out, Auth0 retries once. If both attempts fail, the reconciliation job is the safety net.                                                    |
| **Auth0 Management API rate limits**                                   | Developer plan: ~2 req/s. Free plan: lower. Implement `p-limit` or `bottleneck` in the reconciliation Lambda.                                                    |
...
| **JWKS cache**               | `jwks-rsa` module scope + APIGW 300s TTL   | Secrets Manager cache   | Two-layer minimizes Auth0 fetches and Lambda invocations (§1)                                         |
| **Auth0 deletion retry**     | SQS Standard + Lambda + DLQ                | SNS+SQS, Step Functions | Cheaper, built-in retry via `maxReceiveCount`, exponential backoff via `changeMessageVisibility` (§2) |
