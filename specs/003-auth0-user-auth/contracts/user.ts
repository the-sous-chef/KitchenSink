 ...
 * @description TypeScript interfaces for the User and Account entities stored in Aurora DSQL.
 * These are the application-layer representations — database timestamps are returned as ISO 8601 strings.
 *
 * Source spec: specs/003-auth0-user-auth/spec.md — Key Entities
 * Data model: specs/003-auth0-user-auth/data-model.md
 */
 ...
 * The `id` field is the canonical identifier across all Sous Chef systems — it is a UUIDv4
 * generated at signup and stored in Auth0 `app_metadata.userId`.
 *
 * @see specs/003-auth0-user-auth/spec.md FR-013, FR-015, FR-018
 */
export interface User {
  /** Canonical Sous Chef user ID (UUIDv4). Primary key. Never changes after creation. */
  readonly id: string;
  /**
   * Auth0 `sub` claim (e.g., `auth0|abc123`).
   * Used ONLY for Auth0 Management API calls — never as an application identifier.
   */
  readonly auth0Id: string;
  /**
   * User's email address. Synced from Auth0 at registration. Read-only in Sous Chef;
   * email changes require the Auth0 email change flow.
   */
  ...
  /**
   * Account status. `'suspended'` users are blocked in Auth0 and denied API access.
   * @see specs/003-auth0-user-auth/spec.md FR-041, FR-042
   */
  status: UserStatus;
 ...
 *
 * @see specs/003-auth0-user-auth/spec.md FR-019, FR-020, FR-021
 */
 ...
 *
 * @see specs/003-auth0-user-auth/spec.md FR-014, Key Entities
 */
