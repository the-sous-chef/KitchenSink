# V-Model Peer Review: IdP User Authentication
...
- `specs/002-user-auth/v-model/requirements.md`
- `specs/002-user-auth/v-model/acceptance-plan.md`
- `specs/002-user-auth/v-model/unit-test.md`
- `specs/002-user-auth/v-model/trace.md`
...
> REQ-021: "The account edit page SHALL display the email field as read-only with a note directing users to the IdP for email changes." — Verification Method: **Inspection**
...
- The user.created webhook action creates User/Account records and stores UUIDv4 in `app_metadata` (REQ-013 through REQ-017)
- The API Gateway authorizer validates JWT signature, expiry, audience, and issuer on every request (REQ-038, REQ-039, REQ-042)
- The reconciliation job is the safety net for orphaned IdP users (REQ-017)
...
**Impact**: HAZ-006 (orphaned IdP user), HAZ-008 (unauthorized API access), and HAZ-009 (suspended user retains access) have no unit-level verification. These are rated Critical and High severity in Matrix H.
...
**Evidence**:
Matrix C lists "Account Deletion → IdP Backend API" as a single integration point covering only REQ-024. However, REQ-025 (cascade to all user-owned data) is not listed as a separate integration point, and the cascade behavior (recipes, meal plans, etc.) is not mapped to any integration test.
...
- REQ-NF-001: "IdP SDK types, token interfaces,, API response types" — double comma
- REQ-NF-002: "exported functions, classes, interfaces, type aliases,, interface fields" — double comma
- REQ-CN-008: "shared `@kitchensink/typescript`, `@kitchensink/eslint`, `@kitchensink/prettier`,, `@kitchensink/vitest`" — double comma
...
> REQ-001: "The system SHALL authenticate users via IdP Authorization Code Flow with PKCE on the web platform, the mobile platform."
...
**Resolution**: Rewrite as: "The system SHALL authenticate users via IdP Authorization Code Flow with PKCE on both the web (Next.js) and mobile (Expo) platforms." Alternatively, split into REQ-001-WEB and REQ-001-MOB for unambiguous platform scoping.
...
**Evidence**:
MOD-006 (Mobile IdP Client SDK Wrapper) covers only `getAccessToken` with 4 scenarios (UTP-006-A). The mobile auth lifecycle has at least 4 distinct states: Unauthenticated → Authenticating → Authenticated → Logging Out → Unauthenticated. The unit test plan's own technique table lists "State Transition Testing" as a mandatory technique for State Machine Views.
...
Security-critical behaviors are comprehensively specified: PKCE enforcement (REQ-001), secure token storage (REQ-006), token revocation on logout (REQ-011), JWT validation (REQ-039), suspension enforcement (REQ-042), and password delegation to the IdP (REQ-028, REQ-CN-002). The constraint against using IdP `sub` as primary identifier (REQ-CN-003) demonstrates vendor lock-in awareness.

---

#### PRF-002-P3 — `requirements.md`: Assumptions and Dependencies Section

The Assumptions section correctly identifies pre-conditions (IdP tenant provisioned, database schema defined, CDK environment configured) that are outside the feature's scope. This is good practice for preventing scope creep and clarifying implementation prerequisites.
...
2. **PRF-002-A2** is the most significant gap — the backend Lambda modules (user.created webhook action, API Gateway authorizer, reconciliation job) are the highest-risk components in the feature and have zero unit test coverage defined.
