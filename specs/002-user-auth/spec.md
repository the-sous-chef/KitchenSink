...
- **Email change flow**: Users cannot change their email address through Sous Chef. Email changes require a separate feature that coordinates with the IdP's email verification.
...
A new user opens Sous Chef for the first time and creates an account. On the mobile app, the identity provider (IdP) authentication screen is displayed automatically. On the web app, the user is redirected to the IdP sign-up page. The user provides their email and password (or uses a social provider). After successful signup, the IdP fires a `user.created` webhook that creates a User and Account record in the Sous Chef database. The user is then authenticated and lands on the home screen.
...
3. **Given** a user completes signup with email and password, **When** the IdP fires the `user.created` webhook, **Then** the Sous Chef backend creates a User record (ULID primary key, `identity_id` set to the IdP's `user.id`) and an Account record atomically.
...
A logged-in user navigates to the profile page. The system displays their display name, email, avatar, and account creation date sourced from the Sous Chef database.
...
A logged-in user decides to delete their account. They initiate deletion from the account settings page. The system requires explicit confirmation (e.g., type "DELETE" to confirm). On confirmation, the system deletes the User and Account records from the Sous Chef database AND deletes the user from the identity provider (IdP). This is a permanent, irreversible action. The user is logged out and returned to the authentication screen.
...
**Independent Test**: Can be fully tested by creating a user, initiating account deletion, confirming the action, and verifying the user no longer exists in both the Sous Chef database and the IdP. Attempting to log in with the deleted credentials should fail.
...
4. **Given** the Sous Chef database deletion succeeds but the IdP deletion fails, **When** the system encounters the error, **Then** it proceeds with the local deletion, queues the IdP deletion for automatic async retry (exponential backoff, max 5 attempts), and logs the failure. The user is logged out and returned to the authentication screen without being blocked.
...
A user who has forgotten their password or wants to change it triggers the IdP password reset flow. On the login screen, they click "Forgot Password" which directs them to the IdP's password reset page. The IdP sends a reset email; the user follows the link to set a new password. The entire flow is handled by the IdP — the Sous Chef backend is not involved in password management.
...
4. **Given** the password reset flow, **When** it completes, **Then** the Sous Chef backend has not stored, processed, or had access to the user's password at any point.
...
- **`user.created` webhook**: The IdP fires this webhook on signup. The Sous Chef backend creates User + Account + Profile records atomically. The app-generated ULID is the canonical user ID; the IdP's `user.id` is stored as `identity_id` (secondary key).
...
- **Reconciliation Lambda**: Nightly job that detects IdP users without a corresponding Sous Chef database record and creates the missing records.
...
- **FR-013**: On user signup, the IdP MUST fire a `user.created` webhook that upserts a User record in the Sous Chef database. The User record uses an app-generated ULID as the primary key (`id`); the IdP's `user.id` is stored as `identity_id` (secondary key, unique).
...
- **FR-015**: The app-generated ULID is the canonical Sous Chef user identifier. It is available in all IdP access tokens via the `app_user_id` custom claim (injected by the authorizer after resolving the user from the database). No `app_metadata` writeback is required for the `identity_id` — it is stored in the database and injected by the authorizer.
...
- **FR-017**: A reconciliation mechanism MUST exist to detect IdP users without corresponding Sous Chef database records and create the missing records.
...
- **FR-028**: The password reset flow MUST be handled entirely by the IdP — the Sous Chef backend MUST NOT store, process, or have access to user passwords.

**Social Account Linking**

- **FR-032**: System MUST provide APIs for linking a social identity provider account (e.g., Google) to an existing Sous Chef user account.
- **FR-033**: System MUST provide APIs for unlinking a previously linked social identity provider account from a Sous Chef user account.
- **FR-034**: Linking/unlinking operations MUST be handled via the IdP Backend API. The Sous Chef backend MUST NOT store social provider credentials or tokens.

**API Authorization**

- **FR-038**: All Sous Chef API endpoints MUST require a valid IdP access token. Requests without a valid token MUST receive `401 Unauthorized`.
...
- **FR-041**: The system MUST support suspending a user by both blocking the user in the IdP (via the Backend API) and setting the User entity `status` to `suspended` in the Sous Chef database.
...
- **NFR-011a**: Sous Chef's own auth-related API operations MUST meet the following latency targets: silent token refresh endpoint ≤ 500ms P99, profile data endpoint ≤ 1s P99, webhook processing (from API Gateway receipt to database write) ≤ 2s P99. These targets apply only to Sous Chef-controlled operations; IdP-hosted login page load times are explicitly out of scope.
...
- **SC-004**: Account deletion completes (both Sous Chef database and IdP) within 30 seconds of user confirmation.
...
- **A-001**: The identity provider (IdP) is the sole authentication authority. All credential management (password storage, social identity linking) is handled by the IdP. The Sous Chef backend never stores or processes passwords.
...
- **A-004**: The app-generated ULID is the canonical identifier used across all Sous Chef systems. The IdP's `user.id` is stored as `identity_id` (secondary key) and is used only for IdP Backend API calls.
...
- **A-006**: The Sous Chef database (PostgreSQL, per the USDA spec's architecture) stores User, Account, Profile, and WebhookEvent records. No separate auth database is required.
...
- **A-008**: Session persistence relies on IdP session tokens. The Sous Chef backend does not maintain its own session store.
- **A-009**: The shared API Gateway authorizer referenced in USDA spec FR-035 is the IdP JWT authorizer implemented by this feature. This feature provides the authentication layer that both Sous Chef and USDA food data endpoints depend on.
- **A-010**: Account deletion is a hard delete, not a soft delete. All user data is permanently removed from the Sous Chef database. IdP user deletion is permanent and irreversible.
...
- Q: What JWT custom claim key carries the canonical Sous Chef user ID? → A: `app_user_id` (flat top-level claim) — used by the authorizer and injected into all downstream requests.
...
- Q: Should performance targets be set for auth flows? → A: Yes — targets apply only to Sous Chef-controlled operations: token refresh API ≤ 500ms P99, profile API ≤ 1s P99, webhook processing ≤ 2s P99. IdP-hosted login page load times are out of scope.
