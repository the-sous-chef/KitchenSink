...
**Clerk** as primary identity provider across web and mobile.
...
- Existing spec/plan artifacts are already written around Clerk webhooks + Backend API patterns.

### Dependencies

- Web: `@clerk/nextjs` v4.x
- Mobile: `@clerk/expo` v5.5.0
...
- **SQS deletion queue + DLQ** for Clerk deletion retries
- **EventBridge Scheduler** for nightly Clerk↔DB reconciliation

### Rationale

- Handles transient Management API failures without blocking user-visible account deletion completion.
- Provides deterministic recovery path for user.created webhook/write-failure orphan scenarios.
...
| ------------------------------ | ----------------------------------------- | --------------------------------------------------- |
| Clerk-managed identity         | Fast delivery of complex auth flows       | Vendor dependency and rate-limit management         |
