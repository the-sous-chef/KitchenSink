...
- **Web (Next.js)**: protected routes redirect unauthenticated users to Clerk login.
...
Signup/login UI is delegated to Clerk hosted UI with at least Google + email/password options.
...
Edit screen supports mutable display name and avatar with validation; email is visible but read-only with redirect note to Clerk flow.
...
- Do not block user on downstream Clerk API retry path.
...
Expose “Forgot Password” entry on login. Delegate full reset UX to Clerk hosted flow.
...
Expose MFA enrollment from account settings; Clerk handles second-factor challenge on subsequent logins.
...
1. Launch Clerk browser session.
