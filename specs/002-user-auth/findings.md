...
> **Migration Note (2026-05-26):** This feature was migrated from Auth0 to Clerk. Findings below were written against the Clerk-based design and remain valid unless marked otherwise.
...
Evidence: documents that once issued, access tokens and ID tokens cannot be revoked in the same way as server-side session IDs. JWT access tokens are self-contained; explicitly recommends short lifetimes instead. That means changing `app_metadata`, suspending a user, or revoking refresh tokens does not immediately invalidate an in-flight access token. Revocation lag = token lifetime + authorizer cache TTL.
