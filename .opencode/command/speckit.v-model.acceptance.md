...
Run `.specify/scripts/bash/setup-v-model.sh --json --require-reqs` from the repository root. The `--require-reqs` flag ensures `requirements.md` exists.
...
**Note on script paths**: The helper scripts (`diff-requirements`, `validate-requirement-coverage`) are in the same directory as the setup script. Derive the scripts directory from the `.specify/scripts/bash/setup-v-model.sh --json --require-reqs` path (i.e., its parent directory).
...
2. Read the template overlay: `.specify/templates/overlays/{domain}/acceptance-plan-template.md`
...
1. **Load the template**: Read `.specify/templates/acceptance-plan-template.md` from the extension directory.
...
Where `{SCRIPTS_DIR}` is the parent directory of the `.specify/scripts/bash/setup-v-model.sh --json --require-reqs` path (or the equivalent PowerShell directory).
