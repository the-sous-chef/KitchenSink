...
# Custom output path
.specify/scripts/bash/build-audit-report.sh specs/<feature>/v-model --output /tmp/audit-report.md

# JSON output to stdout (for CI pipelines)
.specify/scripts/bash/build-audit-report.sh specs/<feature>/v-model --json
```
...
# JSON output
.specify/scripts/powershell/Build-Audit-Report.ps1 -VModelDir specs/<feature>/v-model -Json
```
