---
description:
  Build a regulatory-grade Bidirectional Traceability Matrix with five-matrix
...
- **Forward Traceability (REQ → ATP → SCN):** Proves every requirement has been tested. _Ensures no missing features._
- **Backward Traceability (SCN → ATP → REQ):** Proves every test maps back to a justified requirement. _Ensures no gold-plating, undocumented features, or rogue code (a massive security/safety risk)._
...
**Matrix B — Verification (Architectural View)** _(only when system-level artifacts exist)_:
...
**Matrix C — Integration Verification (Module Boundary View)** _(only when architecture-level artifacts exist)_:
...
**Matrix D — Implementation Verification (Module View)** _(only when module-design.md and architecture-design.md exist)_:
